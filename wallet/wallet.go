...
func NewWallet() *Wallet {
	...
	w := new(Wallet)
	privateKey, _ := ecdsa.GenerateKey(elliptic.P256(), rand.Reader) //make sure import is crypto/rand
	...
	digest2 := h2.Sum(nil)
	//3. Perform RIPEMD-160 hashing on the result of the SHA-256 (20 bytes)
	h3 := ripemd160.New()
	h3.Write(digest2)
	digest3 := h3.Sum(nil)
	//4. Add version byte in front of the RIPEMD-160 hash (0x00 for Main Network)
	vd4 := make([]byte, 21) // 21 refers to the number of bytes. h3 is 20 and 0x00 is an additional byte
	vd4[0] = 0x00
	copy(vd4[1:], digest3[:])
	...
	h5.Write(vd4)
	digest5 := h5.Sum(nil)
	//6. Perform SHA-256 hash on the previous hash
	h6 := sha256.New()
	h6.Write(digest5)
	digest6 := h6.Sum(nil)
